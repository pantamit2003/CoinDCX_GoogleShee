...
def _check_breakout_confirmation(df, spike_time, candle_color):
    ...
    Return: (status, n1_high, n1_low) tuple, ya (None, None, None) agar
    abhi determine nahi ho sakta. n1_high/n1_low target-price calculate
    karne ke liye chahiye (stop-loss reference point).
    """
    ...
    if not found_n1 or not found_n2:
        return None, None, None  # abhi itna time nahi guzra
    ...
    if candle_color == "GREEN":
        if broke_high:
            return "CONFIRMED_CONTINUATION", high_n1, low_n1
        elif broke_low:
            return "FAILED_BREAKOUT", high_n1, low_n1
        else:
            return "STILL_UNDECIDED", high_n1, low_n1
    else:
        if broke_low:
            return "CONFIRMED_CONTINUATION", high_n1, low_n1
        elif broke_high:
...
def resolve_pending(dry_run=False):
    ...
    for row in records:
        ...
        if confirmation_status == "PENDING":
            new_status, n1_high, n1_low = _check_breakout_confirmation(df, spike_time, candle_color)
            if new_status is not None:
                ...
                if rvol_20 >= CONFIRMATION_ALERT_RVOL_THRESHOLD:
                    ...
                    # ---- TARGET CALCULATION (sirf CONFIRMED_CONTINUATION ke liye) ----
                    # Stop-loss = N+1 candle ka opposite extreme (jise todne se
                    # breakout FAILED maana jaata — yehi natural invalidation point hai)
                    # Risk = Entry se Stop-loss ki distance
                    # Target = Entry +/- (Risk x Reward_Ratio), 2:1 / 3:1 / 4:1 teeno
                    target_lines = ""
                    if new_status == "CONFIRMED_CONTINUATION" and n1_high is not None:
                        if candle_color == "GREEN":
                            stop_loss = n1_low
        ...
        result_row.append(row.get("Early_Signal", "UNKNOWN"))
    ...
    if not dry_run:
        try:
            clean_rows = [[r["Pair"], r["Trigger_Type"], r["Candle_Color"],
                            ...
                            r.get("Early_Signal", "PENDING")] for r in still_pending]
            pending_ws.clear()
